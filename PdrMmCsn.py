...
def main(args):
    ...
    input_data = pd.DataFrame(input_data.values.T, columns=input_data.index, index=input_data.columns)

    tag = "MM_PDR"
    if args.tag is not None:
        tag = args.tag
    color_file = tag + ".color.txt"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--inputFile', type=str, required=True, help="inputdata gene expression file, .txt format, include 3 row: region/metrics1/metrics2")
    ...
    parser.add_argument('--tag', type=str, required=False, help="prefix of the output file(s)")
